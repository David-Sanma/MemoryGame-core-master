	...
	public void playGame(ArrayList<Integer> arr) {
		...
		while (nRestOfCards > 0)
		{
			...
			do{
				...
				nSecond = sc.nextInt();
				if(nSecond == nFirst){System.out.println("No se puede elegir el mismo número");}
			}while(nSecond == nFirst);

			if(nSecond < 1 || nSecond > arr.size()){
				System.out.println("The value is out of range");
				System.out.println("GAME OVER");
			}
			if(nSecond > 0 && nSecond <= arr.size()){
